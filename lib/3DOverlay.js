...
Overlay.prototype.add_obj_object = function(arguments){
	var self = this;
	var name = arguments.name;
	var meshSrc = arguments.mesh_src;
	var texSrc = arguments.texture_src;
	var offsets = arguments.offsets;
...
Overlay.prototype.add_dae_object = function(arguments){
	var self = this;
	var name = arguments.name;
	var meshSrc = arguments.mesh_src;
	var offsets = arguments.offsets;
	...
	var cup = false;
	if(typeof offsets != "undefined")
		if(typeof offsets.convert_axis != "undefined")
			cup = offsets.convert_axis;
	loader.options.convertUpAxis = cup;
	loader.load(meshSrc, function(collada){
		...
		dae.traverse(function(child){
			...
			if(child instanceof THREE.SkinnedMesh){
				...
				try{
					...
					if(dae.offsets != undefined)
						if(dae.offsets.animate != undefined)
							if(dae.offsets.animate == true)
								animation.play();
					//child.material.opacity = 0;
				}catch(e){
					console.log(e);
		...
		var newPos = self.convert_to_perspective(320,240);
		
		dae.dest_x = 0;
		dae.dest_y = 0;
		console.log(dae.dest_x);
		console.log(dae);
		console.log(dae.dest_x);
...
Overlay.prototype.set_object_position = function(object,position){
	var x, y;
	x = position.x;
	y = position.y;
	if(typeof object == "undefined")
	...
	if(typeof object.offsets != "undefined"){
		if(typeof object.offsets.position != "undefined"){
			if(typeof object.offsets.position.x != "undefined")
				if(!isNaN(object.offsets.position.x))
					x = position.x + object.offsets.position.x * object.scale.x;
			if(typeof object.offsets.position.y != "undefined")
				if(!isNaN(object.offsets.position.y))
					y = position.y += object.offsets.position.y * object.scale.y;
		}
	}
...
Overlay.prototype.convert_to_perspective = function (xVal,yVal){
	var x = Math.floor(xVal * .466666667 - 140);
	var y = Math.floor(yVal * -.577777778 + 130);
	return {"x": x, "y": y};
