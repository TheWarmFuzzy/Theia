...
(function(window, undefined) {
   ...
   * @param {object} opt_options Optional configuration to the tracker.
   */
  tracking.track = function(camera, tracker, opt_options) {
	return this.trackVideo_(camera, tracker, opt_options);
  };
   ...
   * @private
   */
  tracking.track_bool = true;
  tracking.trackVideo_ = function(camera, tracker) {
    var canvas = document.createElement('canvas');
    ...
    var resizeCanvas_ = function() {
      //width = camera.video.offsetWidth;
      //height = camera.video.offsetHeight;
	  width = camera.video.offsetWidth / video_scalar;
      height = camera.video.offsetHeight / video_scalar;
      canvas.width = width;
      canvas.height = height;
    };

    resizeCanvas_();
    camera.video.addEventListener('resize', resizeCanvas_);
...
(function() {
   ...
   * @type {Array.<string>}
   */
  tracking.ColorTracker.prototype.colors = ['theia_red','theia_green'];
   ...
   * @type {number}
   */
  tracking.ColorTracker.prototype.minDimension = 8;
   ...
   * @type {number}
   */
  tracking.ColorTracker.prototype.maxDimension = 150;
   ...
   * @type {number}
   */
  tracking.ColorTracker.prototype.minGroupSize = 20;
  ...
  tracking.ColorTracker.prototype.calculateDimensions_ = function(cloud, total) {
    ...
    return {
      width: Math.floor((maxx - minx) * video_scalar),
      height: Math.floor((maxy - miny) * video_scalar),
      x: Math.floor(minx * video_scalar),
      y: Math.floor(miny * video_scalar)
    };
  };
  ...
  tracking.ColorTracker.prototype.mergeRectangles_ = function(rects) {
    var intersects;
    var results = [];
    var minDimension = this.getMinDimension() * video_scalar;
    var maxDimension = this.getMaxDimension() * video_scalar;
  ...
  tracking.ColorTracker.prototype.trackColor_ = function(pixels, width, height, color) {
    ...
    var results = [];
    var w = -4;
	var currGroup, marked, queue;
	try{
		currGroup = new Int32Array(pixels.length >> 2);
		marked = new Int8Array(pixels.length);
		queue = new Int32Array(pixels.length);
	}catch(e){
		console.log("oops");
		return "restart";
	}
  ...
  tracking.ColorTracker.registerColor('theia_red', function(r, g, b) {
  ...
  tracking.ColorTracker.registerColor('theia_green', function(r, g, b) {
